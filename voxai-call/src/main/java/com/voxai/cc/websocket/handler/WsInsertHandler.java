...
import com.voxai.cc.websocket.event.WsMonitorEvent;
...
public class WsInsertHandler extends WsBaseHandler<WsMonitorEvent> {

    @Override
    public void handleEvent(WsMonitorEvent event) {
        logger.info("agent:{} insert to target:{}", event.getAgentKey(), event.getTarget());
        ...
        if (targetAgent == null) {
            logger.warn("insert failed, target:{} not found", target);
        ...
        if (callInfo == null) {
            logger.warn("insert failed, target:{} not in call", target);
        ...
        callInfo.getNextCommands().add(new NextCommand(supervisorDeviceId, NextType.NEXT_INSERT_CALL, targetDeviceId));
        ...
        sendMessage(event, new WsResponseEntity<>("INSERTING", event.getAgentKey()));
    }
}
